...
create: () => deepseekBalanceSnapshotResult$schema
},
...
/** True on Beijing Saturdays/Sundays: DeepSeek bills weekends at the off-peak rate all day (since 2026-08-23). */
...
const MARQUEE_STYLE_ID = "deepseek-usage-marquee-style";
...
function ensureMarqueeStyle() {
  ...
  style.textContent = [
    "@keyframes deepseekUsageMarquee {",
    "  0%, 10% { transform: translateX(0); }",
    "  90%, 100% { transform: translateX(var(--marquee-shift, -100%)); }",
    "}",
    ".deepseek-usage-marquee:hover .deepseek-usage-marquee-track {",
    "  animation: deepseekUsageMarquee 9s linear infinite;",
    "}"
  ].join("\n");
 ...
 *   [blue whale] Balance ¥x · Peak          (Beijing weekday peak window)
 *   [theme whale] Balance ¥x · Off-peak 50% (otherwise, incl. all weekend)
 * refreshed every 60 seconds, and links to the DeepSeek platform. When the
 * content is wider than the chip, hovering scrolls it marquee-style so the
 * tail stays readable; the full text is always in the title tooltip.
 */
...
const [chipCap, setChipCap] = React.useState(CHIP_MAX_WIDTH);
  ...
  React.useEffect(() => {
    ...
    const measure = () => {
      ...
      const clip = el.querySelector(".deepseek-usage-marquee-clip");
      const track = el.querySelector(".deepseek-usage-marquee-track");
    ...
    measure();
    const el = chipRef.current;
    const clip = el ? el.querySelector(".deepseek-usage-marquee-clip") : null;
...
className: "deepseek-usage-marquee",
style: {
display: "inline-flex",
alignItems: "center",
gap: "6px",
...
key: "clip",
className: "deepseek-usage-marquee-clip",
style: {
...
React.createElement("span", {
  key: "track",
  className: "deepseek-usage-marquee-track",
  style: {
    display: "inline-flex",
    alignItems: "center",
    whiteSpace: "nowrap",
